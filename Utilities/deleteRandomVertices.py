...
# then it create the complement for each dimension i.e edges that are not in G_1 or G_2
#
# January 2017
...
for edge in itertools.combinations(V, 2):
    if edge in E_dimension1 and edge in E_dimension2:
        new_edges.append('<edge source="%s" target="%s" %s="1" %s="2"/>' % (edge[0], edge[1], dimension1,dimension2))
    elif edge in E_dimension1 and edge not in E_dimension2:
        new_edges.append('<edge source="%s" target="%s" %s="1" c_%s="4"/>' % (edge[0], edge[1], dimension1,dimension2))
    elif edge in E_dimension2 and edge not in E_dimension1:
        new_edges.append('<edge source="%s" target="%s" %s="2" c_%s="3"/>' % (edge[0], edge[1], dimension2, dimension1))
    else:
        new_edges.append('<edge source="%s" target="%s" c_%s="3" c_%s="4"/>' % (edge[0], edge[1], dimension1,dimension2))
    counter += 1
